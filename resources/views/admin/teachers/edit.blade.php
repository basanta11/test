...
<div class="kt-container  kt-grid__item kt-grid__item--fluid">
	...
	<div class="kt-portlet kt-portlet--mobile">
        ...
        <div class="kt-portlet__body">
            <form class="kt-form" action="/teachers/{{ $teacher->id }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
                <div class="kt-portlet__body p-0 pb-4">
                    <div class="form-group">
                        <label>{{ __("Name") }}</label>
                    <input type="text" name="name" value="{{ old('name',$teacher->name )}}" class="form-control" placeholder="{{ __("Enter name") }}">
                        {{-- <span class="form-text text-muted">We'll never share your email with anyone else.</span> --}}
                    </div>
                    <div class="form-group">
                        <label>{{ __("Email address") }}</label>
                        <input type="email" name="email" value="{{ old('email',$teacher->email )}}"  class="form-control" aria-describedby="emailHelp" placeholder="{{ __("Enter email") }}">
                        {{-- <span class="form-text text-muted">We'll never share your email with anyone else.</span> --}}
                    </div>
                    <div class="form-group">
                        <label>{{ __("Address") }}</label>
                        <input type="text" name="address" autocomplete="off" value="{{ old('address',$teacher->address )}}"  class="form-control" placeholder="{{ __("Enter address") }}">
                    </div>
                    <div class="form-group">
                        <label>{{ __("House number") }}</label>
                        <input type="text" name="house_number" autocomplete="off" value="{{ old('house_number',$teacher->house_number )}}" class="form-control" placeholder="{{ __("Enter house number") }}">
                    </div>
                    <div class="form-group">
                        <label>{{ __("Phone") }}</label>
                        <input type="number" name="phone" min="1" value="{{ old('phone',$teacher->phone )}}"  class="form-control" placeholder="{{ __("Enter phone") }}">
                    </div>
                    <div class="form-group">
                        <label>{{ __("Citizenship Number") }}</label>
                        <input value="{{ $teacher->citizen_number }}" type="text" name="citizen_number" class="form-control" placeholder="{{ __("Enter citizen number") }}">
                    </div>
                    <div class="form-group">
                        <label>{{ __("Symbol Number") }}</label>
                        <input value="{{ $teacher->symbol_number }}" type="text" name="symbol_number" class="form-control" placeholder="{{ __("Enter symbol number") }}">
                    </div>
                    <div class="form-group">
                        <label>{{ __("Gender") }}</label>
                        <select name="gender" class="form-control" required>
                            <option selected disabled>{{ __("Select gender") }}...</option>
                            <option {{ $teacher->gender==0 ? 'selected' :'' }} value="0">{{ __("Male") }}</option>
                            <option {{ $teacher->gender==1 ? 'selected' :'' }} value="1">{{ __("Female") }}</option>
                            <option {{ $teacher->gender==2 ? 'selected' :'' }} value="2">{{ __("Other") }}</option>
                        </select>
                    </div>
                    <div class="form-group">
