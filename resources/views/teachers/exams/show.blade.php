                                ...
                                <div class="kt-widget__content pl-0">
                                    <div class="kt-widget__head">
                                        <a href="#" class="kt-widget__username">
                                            {{ $exam->title }}
                                            @if($exam->status!=1) <i class="text-danger flaticon2-exclamation"></i> @else <i class="flaticon2-correct"></i> @endif
                                        </a>
                                        <div class="kt-widget__action d-none">

                                            <a href="/exams/{{ $exam->id }}/edit"><button type="button"  class="btn btn-warning btn-sm btn-upper">{{ __("Edit") }}</button></a>&nbsp;
                                            
                                            @if($exam->status!=1)
                                                <button type="button" data-status="{{ $exam->status }}" href="#" data-id="{{ $exam->id }}" class="status-change-course btn btn-label-success btn-sm btn-upper">{{ __("Activate") }}</button>&nbsp;
                                            @else
                                                <button type="button" data-status="{{ $exam->status }}" href="#" data-id="{{ $exam->id }}" class="status-change-course btn btn-label-danger btn-sm btn-upper">{{ __("Deactivate") }}</button>&nbsp;
		...
		columns: [

			{
				...
				template:function(data){
					return `<a href="/sets/${data.id}"><span>${data.title}</span></a>`
				}
			},
            {
				field: 'type',
				...
				template: function(data) {
					...
					<div class="dropdown">\
						...
						<div class="dropdown-menu dropdown-menu-right">\
                            <a class="dropdown-item" href="/sets/'+data.id+'"><i class="la la-eye"></i> {{ __("View") }}</a>\
						</div>\
					</div>\
				';
				},
