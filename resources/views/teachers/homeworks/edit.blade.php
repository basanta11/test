...
<div class="kt-container  kt-grid__item kt-grid__item--fluid">
	...
	<div class="kt-portlet kt-portlet--mobile">
        ...
        <div class="kt-portlet__body">
            <form class="kt-form" id="homework_form" method="POST" action="/homeworks/{{ $homework->id }}">
                @csrf
                @method('PATCH')
                ...
                <div class="kt-portlet__body p-0 pb-4">
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6">
                            ...
                            <select name="course_id" class="form-control">
                                ...
                                <option @if($homework->course_id==$course->id) selected @endif value="{{ $course->id }}">{{ $course->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-sm-6">
                            ...
                            <select name="section_id[]" multiple="" class="form-control">
                                @foreach($sections as $section)
                                    <option @if(in_array($section['section']['id'], $homework_sections)) selected @endif value="{{ $section['section']['id'] }}">{{ $section['section']['title'] }}</option>
                    ...
                    <div class="form-group row" >
                        ...
                        <div class="col-sm-6">
                            ...
                            <input type="number" name="full_marks" value="{{ $homework->full_marks }}" required min="1" class="form-control" placeholder="{{ __("Enter full marks") }}">
                        </div>
                        <div class="col-lg-1">
                            <span class="kt-switch kt-switch--outline kt-switch--icon kt-switch--success">
                                <label class="mb-2">
                                    {{ __("Due Date") }}?
                                    <input name="has_due" type="checkbox" {{ !empty($homework->due_date_time) ? 'checked' : '' }} id="due-date-check">
                                    <span></span>
                                </label>
                            </span>
                        </div>
                        <div class="col-lg-5" id="due-date-div">
                            ...
                            <input type="text" id="kt_datepicker_1" class="form-control datepicker-mem" value="{{ $homework->due_date_time }}" placeholder="{{ __("Enter due date") }}" name="due_date_time" autocomplete="off">
                        </div>
                    </div>
                    <div class="form-group">
                        <label>{{ __("Title") }}</label>
                        <input class="form-control" type="text" name="title" value="{{ $homework->title }}" required class="form-control" placeholder="{{ __("Enter title") }}">
                    </div>
                    <div class="form-group">
                        <label>{{ __("Question") }}</label>
                        <textarea type="text" name="question" id="question"  required class="form-control" placeholder="{{ __("Enter question") }}"> {{ $homework->question }}</textarea>
                    </div>
                </div>
            </form>
...
<script>
    ...
    @if (empty($homework->due_date_time)) $('#due-date-div').hide(); @endif
